...
def add_model_options(parser):
    ...
    group.add_argument("--num_heads", default=32, type=int, help="Number of heads in the transformer model.")
...
def add_data_options(parser):
    ...
    group.add_argument("--dataset", choices=['humanml', 'kit', 'humanact12', 'uestc'], type=str,
...
def add_evaluation_options(parser):
    ...
    group.add_argument("--model_path", required=True, type=str,
                       help="Path to model####.pt file to be sampled.")
    group.add_argument("--eval_mode", default='wo_mm', choices=['wo_mm', 'mm_short', 'debug', 'full', 'fast'], type=str,
...
def evaluation_parser(parser=None):
    # parser = ArgumentParser()
    # args specified by the user: (all other will be loaded from the model)
    add_base_options(parser)
