...
if __name__ == '__main__':
    import os
    import pickle
    for fn in os.listdir('/tmp/s'):
        with open(os.path.join('/tmp/s', fn), 'rb') as f:
            data = pickle.load(f)

        plot_3d_motion(os.path.join('/tmp/s2', data['filename']), data['skeleton'], data['motion'], dataset='humanact12', title=data['title'], fps=data['fps'])