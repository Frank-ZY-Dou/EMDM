    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                ...
                # print(f'{motion_loader_name}: {sent_lens}')
...
def evaluate_humanml(args):
    ...
    return evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=run_mm)
        ...
        for i in range(args.start, args.end+1, args.interval):
            ...
            for key, value in result.items():
                if key not in results:
                    results[key] = []
                results[key].append((i, value))

            # results.append((i, result['FID_vald']))
        print('results:', results)
        for key, value in results.items():
            print(key)
            for i, v in value:
                if 'R_precision' in key:
