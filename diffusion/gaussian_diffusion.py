...
from diffusion.sampling import Posterior_Coefficients
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        self.lambda_rot_mse = lambda_rot_mse
        ...
        assert self.num_timesteps in [1, 5, 10, 20, 50], 'Only these values are supported for now!'
        self.posterior_variance = \
            (
                    betas * (1.0 - self.alphas_cumprod_prev) / (1.0 - self.alphas_cumprod)
            ) if self.num_timesteps != 1 else np.array([0., 0.])  # otherwise evaluation when timestep=1 won't run


        # use posterior coefficients from DDGAN, not MDM
        posterior_coe = Posterior_Coefficients(args, device='cpu')
        self.posterior_log_variance_clipped = posterior_coe.posterior_log_variance_clipped.numpy()
        self.posterior_mean_coef1 = posterior_coe.posterior_mean_coef1.numpy()
        self.posterior_mean_coef2 = posterior_coe.posterior_mean_coef2.numpy()
    ...
    def p_mean_variance(
        ...
        model_output = model(x, self._scale_timesteps(t), latent_z, **model_kwargs)

        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            inpainting_mask, inpainted_motion = model_kwargs['y']['inpainting_mask'], model_kwargs['y'][
                'inpainted_motion']
    ...
    def training_losses(self, model, x_tp1, x_start, t, model_output, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        target = x_start
        # print('shapes:', model_output.shape, target.shape, x_start.shape)
        ...
        if self.lambda_rcxyz > 0.:
            ...
            terms["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz,
        ...
        terms["loss"] = self.lambda_rot_mse * terms["rot_mse"] \
                        + terms.get('vb', 0.) + \
                        (self.lambda_vel * terms.get('vel_mse', 0.)) + \
                        (self.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                        (self.lambda_fc * terms.get('fc', 0.))
        terms["unbiased_loss"] = (terms["rot_mse"]
                                  + terms.get('vb', 0.) +
                                  (terms.get('vel_mse', 0.)) +
                                  (terms.get('rcxyz_mse', 0.)) +
                                  (terms.get('fc', 0.))).detach()  # no weights
    ...
    def fc_loss_rot_repr(self, gt_xyz, pred_xyz, mask):
        ...
        pred_joint_vel = torch.linalg.norm(pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1],
                                           axis=2)  # [BatchSize, 4, Frames]
    ...
    def velocity_consistency_loss_humanml3d(self, target, model_output):
        ...
        for i in range(21):
            plt.plot(
                np.linalg.norm(calc_vel_from_xyz[:, i * 3:(i + 1) * 3, :, :].cpu().detach().numpy().reshape((3, -1)),
                               axis=0), label='Calc Vel')
            plt.plot(
                np.linalg.norm(velocity_from_vector[:, i * 3:(i + 1) * 3, :, :].cpu().detach().numpy().reshape((3, -1)),
                               axis=0), label='Vector Vel')
