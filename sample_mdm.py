...
from diffusion.sampling import sample_from_model, Posterior_Coefficients
import argparse
...
def main():
    ...
    args = generate_args(parser)
    fixseed(args.seed)
    out_path = ''#rf'./save/{args.dataset}/{args.exp}'.replace(' ', '_')
    ...
    for rep_i in range(args.num_repetitions):
        ...
        # print("model_kwargs['y']:", model_kwargs['y'])

        # print('y:', model_kwargs['y'].keys())
        ...
        x_init = torch.randn(args.batch_size, *dataset_param['input_size'], device=device)
        import time
        t0 = time.time()
        sample = sample_from_model(pos_coeff, model, args.num_timesteps, x_init, None, args, model_kwargs['y'])
        print(f'sample_from_model took {time.time() - t0} seconds')
        # (
        #     sample_fn(
        #     model,
        ...
        # currently [batch, 24, 3, 60]
        # transform to [batch, 60, 24, 3]
        sample = sample.permute(0, 3, 1, 2)
        print('before filter, shape:', sample.shape)
        for i in range(sample.shape[0]):
            sample[i] = torch.tensor(motion_temporal_filter(sample[i].cpu().numpy(), sigma=2.5))
        # transform back to [batch, 24, 3, 60]
        sample = sample.permute(0, 2, 3, 1)
        print('after filter, shape:', sample.shape)
        # END FILTER
